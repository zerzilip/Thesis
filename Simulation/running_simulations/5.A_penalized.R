...
for (i in 1:length(n_pred)){
  ...
  start.time <- Sys.time()
  results <- run_simulation(n_t, n_sim, n_external, generate_data_from_logistic,
                            performance_measures, models_list, seed = 8008*i,
                            pooled = TRUE, beta = beta)
...
save(all_results, file = "data/penalized_A_uncor.RData")
