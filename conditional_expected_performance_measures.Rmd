...
```
As we can see all the performance measures except the slope behaves similarly, their average does not depend on the sample size. Howeve, for the calibration slope, it shows an assymptotic behaviour. 
...
```{r, eval = FALSE}
png('../Figures/test.png', width = 18, height = 22, units = 'cm', res = 400)
grid.arrange(p1, p2, p3, p4, ncol = 2, nrow = 2)
dev.off()
```
